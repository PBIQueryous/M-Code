...
let
                ...
                //Date table code
                StartDate = Date.From("01/01/" & Text.From(StartYearNUM)),
                EndDate = Date.From("31/12/" & Text.From(EndYearNUM)),
                ...
                tx_AddToday = if EndDate < var_CurrentDate then List.Combine( {list_Dates, {var_CurrentDate}}) else list_Dates,
                ...
                col_YearOFFSET = Table.AddColumn(col_Year, "CurrYearOffset", each Date.Year([Date]) - Date.Year( Date.From(var_CurrentDate)), type number),
                col_isYearComplete = Table.AddColumn(col_YearOFFSET, "isYearComplete", each Date.EndOfYear([Date]) < Date.From( Date.EndOfYear(var_CurrentDate)), type logical),
                ...
                col_Quarter_Year = Table.AddColumn(col_QuarterEND, "Quarter & Year", each "Q" & Number.ToText( Date.QuarterOfYear([Date])) & Date.ToText([Date], [Format = " yy"]), type text),
                col_QuarterYearINT = Table.AddColumn(col_Quarter_Year, "QuarterYearINT", each [YearNUM] * 10 + [QuarterNUM], type number),
                col_QuarterOFFSET = Table.AddColumn(col_QuarterYearINT, "CurrQuarterOffset", each ((4 * Date.Year([Date])) +  Date.QuarterOfYear([Date])) - ((4 * Date.Year(Date.From(var_CurrentDate))) +  Date.QuarterOfYear(Date.From(var_CurrentDate))), type number),
                col_isQuarterComplete = Table.AddColumn(col_QuarterOFFSET, "isQuarterComplete", each 
let
...
in isQtrComplete
  , type logical),
                ...
                col_CalendarMONTH = Table.AddColumn(col_MonthEND, "Month & Year", each Text.Proper( Date.ToText([Date], [Format = "MMM yy"])), type text),
                col_MonthYearINT = Table.AddColumn(col_CalendarMONTH , "MonthYearINT", each [YearNUM] * 100 + [MonthNUM], type number),
                col_MonthOFFSET = Table.AddColumn(col_MonthYearINT, "CurrMonthOffset", each ((12 * Date.Year([Date])) +  Date.Month([Date])) - ((12 * Date.Year(Date.From(var_CurrentDate))) +  Date.Month(Date.From(var_CurrentDate))), type number),
                col_isMonthComplete = Table.AddColumn(col_MonthOFFSET, "isMonthComplete", each Date.EndOfMonth([Date]) < Date.From(Date.EndOfMonth(var_CurrentDate)), type logical),
                ...
                col_DayMonthNUM = Table.AddColumn(col_MonthNameINITIAL, "DayMonthNUM", each Date.Day([Date]), type number),
                ...
                col_CalendarWEEK = Table.AddColumn(col_WeekEND, "Week & Year", each "W" & Text.PadStart( Text.From( [Week Number] ), 2, "0") & " " & Text.End(Text.From(Date.Year( Date.AddDays( Date.StartOfWeek([Date], Day.Monday), 3 ))), 2), type text ),
                col_WeekYearINT = Table.AddColumn(col_CalendarWEEK, "WeekYearINT", each Date.Year( Date.AddDays( Date.StartOfWeek([Date], Day.Monday), 3 )) * 100 + [Week Number],  Int64.Type),
                col_WeekOFFSET = Table.AddColumn(col_WeekYearINT, "CurrWeekOffset", each (Number.From(Date.StartOfWeek([Date], Day.Monday))-Number.From(Date.StartOfWeek(var_CurrentDate, Day.Monday)))/7, type number),
                col_isWeekComplete = Table.AddColumn(col_WeekOFFSET, "WeekCompleted", each Date.EndOfWeek( [Date], Day.Monday) < Date.From(Date.EndOfWeek(var_CurrentDate, Day.Monday)), type logical),
            
                col_DayWeekNUM = Table.AddColumn(col_isWeekComplete, "DayWeekNUM", each Date.DayOfWeek([Date], Day.Monday) + WDStart, Int64.Type),
                col_DayNAME = Table.AddColumn(col_DayWeekNUM, "Day of Week Name", each Text.Proper( Date.ToText([Date], "dddd" )), type text),
                col_DayINITIAL = Table.AddColumn(col_DayNAME, "Day Initial", each Text.Proper(Text.Start([Day of Week Name], 1)) & Text.Repeat( Character.FromNumber(8203), Date.DayOfWeek([Date], Day.Monday) + WDStart ), type text),
                col_DayYearNUM = Table.AddColumn(col_DayINITIAL, "DayYearNUM", each Date.DayOfYear([Date]), Int64.Type),
                col_DayMonthYearINT = Table.AddColumn(col_DayYearNUM, "DateINT", each [YearNUM] * 10000 + [MonthNUM] * 100 + [DayMonthNUM], type number),
                ...
                col_FY = Table.AddColumn(col_ISOQuarterYearINT, "Fiscal Year",  
each let 
...
"FY" & result1
, type text),
    col_AY = Table.AddColumn(col_FY, "Academic Year",  
each let 
...
, type text),
                ...
                col_FQ = Table.AddColumn(col_AY, "Fiscal Quarter", each "FQ" & Text.From( Number.RoundUp( Date.Month( Date.AddMonths( [Date], - (FYStartMonth -1) )) / 3 )) & " " & (if [MonthNUM] >= FYStartMonth and FYStartMonth >1 then Text.End(Text.From([YearNUM] +0),2) else 
Text.End(Text.From([YearNUM]-1),2)), type text),
    col_AQ = Table.AddColumn(col_FQ, "Academic Quarter", each "AQ" & Text.From( Number.RoundUp( Date.Month( Date.AddMonths( [Date], - (AYStartMonth -1) )) / 3 )) & " " & (if [MonthNUM] >= AYStartMonth and AYStartMonth >1 then Text.End(Text.From([YearNUM] +0),2) else 
Text.End(Text.From([YearNUM]-1),2)), type text),
                col_FQtrYrINT = Table.AddColumn(col_AQ, "FiscalQuarterYearINT", each (if [MonthNUM] >= FYStartMonth and FYStartMonth >1 then [YearNUM] +0 else [YearNUM]-1) * 10 + Number.RoundUp( Date.Month( Date.AddMonths( [Date], - (FYStartMonth -1) )) / 3 ), type number),
    col_AQtrYrINT = Table.AddColumn(col_FQtrYrINT, "AcademicQuarterYearINT", each (if [MonthNUM] >= AYStartMonth and AYStartMonth >1 then [YearNUM] +0 else [YearNUM]-1) * 10 + Number.RoundUp( Date.Month( Date.AddMonths( [Date], - (AYStartMonth -1) )) / 3 ), type number),
                col_FPNUM = Table.AddColumn(col_AQtrYrINT, "FiscalPeriodNUM", each 
let 
arg1 = [MonthNUM] >= FYStartMonth and FYStartMonth >1,
...
in
 result, type number),
    col_APNUM = Table.AddColumn(col_FPNUM, "AcademicPeriodNUM", each 
let 
arg1 = [MonthNUM] >= AYStartMonth and AYStartMonth >1,
...
in
 result, type number),
                col_FYPeriod = Table.AddColumn(col_APNUM, "Fiscal Period", each "FP" & Text.PadStart( Text.From([FiscalPeriodNUM]), 2, "0") & " " & (if [MonthNUM] >= FYStartMonth and FYStartMonth >1 then Text.End(Text.From([YearNUM] +0),2) else Text.End(Text.From([YearNUM]-1), 2)), type text),
    col_AYPeriod = Table.AddColumn(col_FYPeriod, "Academic Period", each "AP" & Text.PadStart( Text.From([AcademicPeriodNUM]), 2, "0") & " " & (if [MonthNUM] >= AYStartMonth and AYStartMonth >1 then Text.End(Text.From([YearNUM] +0),2) else Text.End(Text.From([YearNUM]-1), 2)), type text),
                col_FPYrINT = Table.AddColumn(col_AYPeriod , "FiscalPeriodYearINT", each (if [MonthNUM] >= FYStartMonth and FYStartMonth >1 then [YearNUM] +0 else [YearNUM]-1) * 100 + [FiscalPeriodNUM], type number),
    col_APYrINT = Table.AddColumn(col_FPYrINT , "AcademicPeriodYearINT", each (if [MonthNUM] >= AYStartMonth and AYStartMonth >1 then [YearNUM] +0 else [YearNUM]-1) * 100 + [AcademicPeriodNUM], type number),
    ...
    var_AcademicCalendarSTART = #date( Date.Year(StartDate)-1, AYStartMonth, 1 ),
                col_FiscalFirstDay = Table.AddColumn( col_APYrINT, "FiscalFirstDate", each if [MonthNUM] >= FYStartMonth and FYStartMonth >1 then #date( Date.Year([Date])+0, FYStartMonth, 1) else #date( Date.Year([Date])-1, FYStartMonth, 1), type date ),
    col_AcademicFirstDay = Table.AddColumn( col_FiscalFirstDay, "AcademicFirstDate", each if [MonthNUM] >= AYStartMonth and AYStartMonth >1 then #date( Date.Year([Date])+0, AYStartMonth, 1) else #date( Date.Year([Date])-1, AYStartMonth, 1), type date ),

                var_Table = Table.FromList( List.Transform( {Number.From(var_AcademicCalendarSTART) .. Number.From(EndDate)}, Date.From), Splitter.SplitByNothing(), type table [DateFW = Date.Type]),
                col_FFD = Table.AddColumn( var_Table, "FiscalFirstDay", each if Date.Month([DateFW]) < FYStartMonth then #date(Date.Year([DateFW])-1, FYStartMonth, 1) else #date(Date.Year([DateFW]) + 0, FYStartMonth, 1)),
                col_FWSD = Table.AddColumn( col_FFD, "FWStartDate", each Date.AddYears(Date.StartOfWeek([DateFW], Day.Monday), 0)),
                ...
                join_Date_DateFW = Table.Join( col_AcademicFirstDay, {"Date"}, cols_Expand2, {"DateFW"}, JoinKind.LeftOuter, JoinAlgorithm.SortMerge ),
    ...
    var_FYAYTest = var_FYLogicTest = true and var_AYLogicTest = true,
                txt_Replace = if var_AYLogicTest then Table.ReplaceValue(join_Date_DateFW, each [Fiscal Week Number], each if FYStartMonth =1 then [Week Number] else [Fiscal Week Number], Replacer.ReplaceValue, {"Fiscal Week Number"}) else join_Date_DateFW,
                col_FYW = Table.AddColumn( txt_Replace, "Fiscal Week", each if var_AYLogicTest then "F" & [#"Week & Year"] else if FYStartMonth =1 then "FW" & Text.PadStart( Text.From([Fiscal Week Number]), 2, "0") & Date.ToText(Date.AddYears([Date],0), " yy") else if Date.Month([Date]) < FYStartMonth then "FW" & Text.PadStart( Text.From([Fiscal Week Number]), 2, "0") & Date.ToText(Date.AddYears([Date],-1), " yy") else "FW" & Text.PadStart(Text.From([Fiscal Week Number]), 2, "0") & " " & Text.End(Text.From( Date.Year([Date])+0),2), type text),
                col_FWYearINT = Table.AddColumn(col_FYW, "FiscalWeekYearINT", each if var_FYLogicTest then [WeeknYear] else (if FYStartMonth =1 then Date.Year([Date]) else if Date.Month([Date]) < FYStartMonth then Date.Year([Date])-1 else Date.Year([Date])+0) * 100 + [Fiscal Week Number],  Int64.Type),
    ...
    col_AcademicYearOFFSET = Table.AddColumn(col_FiscalYearOFFSET, "Academic CurrYearOffset", each try (if [MonthNUM] >= AYStartMonth then [YearNUM]+1 else [YearNUM]) - (if var_CurrMonth >= AYStartMonth then var_CurrYear+1 else var_CurrYear) otherwise null, type number),
                col_isCurrFY = Table.AddColumn(col_AcademicYearOFFSET, "IsCurrentFY", each if [Fiscal Year] = var_CurrFY then true else false, type logical),
    col_isCurrAY = Table.AddColumn(col_isCurrFY, "IsCurrentAY", each if [Academic Year] = var_CurrAY then true else false, type logical),
                col_isCurrFQ = Table.AddColumn(col_isCurrAY, "IsCurrentFQ", each if [FiscalQuarterYearINT] = var_CurrFQ then true else false, type logical),
    col_isCurrAQ = Table.AddColumn(col_isCurrFQ, "IsCurrentAQ", each if [AcademicQuarterYearINT] = var_CurrAQ then true else false, type logical),
                col_isCurrFP = Table.AddColumn(col_isCurrAQ, "IsCurrentFP", each if [FiscalPeriodYearINT] = var_CurrFP then true else false, type logical),
    col_isCurrAP = Table.AddColumn(col_isCurrFP, "IsCurrentAP", each if [AcademicPeriodYearINT] = var_CurrAP then true else false, type logical),
                col_isCurrFW = Table.AddColumn(col_isCurrAP, "IsCurrentFW", each if [FiscalWeekYearINT] = var_CurrFW then true else false, type logical),
                col_isPrevYTD = Table.AddColumn(col_isCurrFW, "IsPYTD", each if var_CurrYear-1 = [YearNUM] and [DayYearNUM] <= rec_CurrentDate{0}[DayYearNUM] then true else false, type logical),
                    list_PrevFiscalYearDates = List.Buffer( Table.SelectRows( Table.ExpandTableColumn( Table.NestedJoin(
                        ...
                        Table.AddIndexColumn( Table.RenameColumns( Table.TransformColumnTypes( Table.FromList( List.Dates( Date.AddYears( var_PFFD, +0), Number.From( var_PFFD - Date.AddYears( var_PFFD, -1)),#duration(1,0,0,0)), Splitter.SplitByNothing()),{{"Column1", type date}}), {{"Column1", "DateFY"}}), "Index", 1, 1)
                        , {"Index"}, "Table", JoinKind.LeftOuter), "Table", {"DateFY"}, {"PrevDateFY"}), each [DateFY] <= var_CurrentDate)[PrevDateFY] ),
    ...
    col_isPrevAYTD = Table.AddColumn(col_isPrevFYTD, "IsPAYTD", each if [Academic CurrYearOffset] = -1 then true else false, type logical),
                col_NetWorkDays = if AddRelativeNetWorkdays = true then Table.AddColumn(col_isPrevAYTD, "Relative Networkdays", each fxNETWORKDAYS( StartDate, [Date], Holidays ), type number ) else col_isPrevAYTD,
                fxNETWORKDAYS = (StartDate, EndDate, optional Holidays as list) =>
                    ...
                    in
                    CountDays,
                cols_RemoveToday = Table.RemoveColumns( if EndDate < var_CurrentDate then Table.SelectRows(col_NetWorkDays, each ([Date] <> var_CurrentDate)) else col_NetWorkDays, {"DayYearNUM", "FiscalFirstDate"}), 
                cols_Format = Table.TransformColumnTypes(cols_RemoveToday,{{"YearNUM", Int64.Type}, {"QuarterNUM", Int64.Type}, {"MonthNUM", Int64.Type}, {"DayMonthNUM", Int64.Type}, {"DateINT", Int64.Type}, {"DayWeekNUM", Int64.Type}, {"ISO CurrYearOffset", Int64.Type}, {"ISO QuarterYearINT", Int64.Type}, {"ISO CurrQuarterOffset", Int64.Type}, {"Week Number", Int64.Type}, {"WeekYearINT", Int64.Type}, {"MonthYearINT", Int64.Type}, {"QuarterYearINT", Int64.Type}, {"FiscalQuarterYearINT", Int64.Type}, {"FiscalPeriodNUM", Int64.Type}, {"FiscalPeriodYearINT", Int64.Type}, {"CurrWeekOffset", Int64.Type}, {"CurrMonthOffset", Int64.Type}, {"CurrQuarterOffset", Int64.Type}, {"CurrYearOffset", Int64.Type}, {"Fiscal CurrYearOffset", Int64.Type}, {"FiscalWeekYearINT", Int64.Type}}),
