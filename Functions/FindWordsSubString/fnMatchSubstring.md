...
let
  customFunction =  // fnMatchTextSubstrings                 
/* ------------------------------ 
  Author: Imran Haq
  AKA: PBI QUERYOUS
  Description: fnMatchTextSubstrings
 ---------------------------------*/
...
// checks if WordList is null, then returns a pre-defined list of your choice  
      ListWords =                                                                        
      if InsertList <> null then InsertList else if WordList = null then InsertList else Text.Split(WordList, ","),
      Result =  // check list of substrings                               
      List.AnyTrue(
        // if column text contains 
        List.Transform(ListWords, (substring) => Text.Contains((Column), substring, Comparer.OrdinalIgnoreCase)  // these substrings                   
        )
      )
    in
      Result
...
// 4.0: edit function metadata here
      ...
      [  
          ...
          Documentation.Name = " fnMatchTextSubstrings ", 
          ...
          Documentation.Examples = 
          {
            ...
            Code    = " fnMatchTextSubstrings( [ColumName], WordList, null ) ", 
            Result  = 
"
 1. Create New Function Step
 2. Type function and complete parameters
    2a. fnMatchTextSubstrings( [ColumName], {Canada,Germany}, null )
    2b. fnMatchTextSubstrings( [ColumName], {Canada,Germany}, ListOfWords )
    2c. fnMatchTextSubstrings( [ColumName], null, ListOfWords )
