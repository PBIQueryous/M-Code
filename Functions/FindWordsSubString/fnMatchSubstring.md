...
let
        ...
        // //-- Date table code
          ...
          var_PreviousDate = Date.AddYears(Date.From(DateTime.FixedLocalNow()),-1), 
          ...
          var_FFD = rec_CurrentDate{0}[FYStartDate], 
          ...
          list_PrevAcademicYearDates = List.Buffer(
            Table.SelectRows(
              Table.ExpandTableColumn(
                Table.NestedJoin(
                  Table.AddIndexColumn(
                  ...
                  Table.AddIndexColumn(
                    Table.RenameColumns(
                      Table.TransformColumnTypes(
                        Table.FromList(
                          List.Dates(
                            Date.AddYears(var_PAFD, + 0), 
                            Number.From(var_PAFD - Date.AddYears(var_PAFD, - 1)), 
                            #duration(1, 0, 0, 0)
                          ), 
                          Splitter.SplitByNothing()
                        ), 
          ...
          col_isPrevFYTD = Table.AddColumn(
            ...
            "IsPreviousFYTD", 
            each 
              if [FiscalYearOFFSET] = -1 and [Date] <= var_PreviousDate then
                true
              else
                false, 
            type logical
          ...
          col_isPrevAYTD = Table.AddColumn(
            ...
            "IsPreviousAYTD", 
            each 
              if [AcademicYearOFFSET] = -1 and [Date] <= var_PreviousDate then
                true
              else
                false, 
            type logical
          ...
          col_isPrevAY = Table.AddColumn(
            col_isPrevFY, 
            "IsPreviousAY", 
            each if [AcademicYearOFFSET] = -1 and List.Contains(list_PrevAcademicYearDates, [Date]) then true else false, 
            type logical
          ), 
          col_NetWorkDays = 
    ...
    list_FieldHeaders = Table.ColumnNames( cols_Reorder ),
    list_FiscalCols = List.Select( list_FieldHeaders , each Text.Contains( _, "Fiscal" ) or Text.Contains( _, "CurrentF" ) or Text.Contains( _, "PreviousF" ) or Text.Contains( _, "DateF" ) or Text.Contains( _, "FY" )),
    list_AcademicCols = List.Select( list_FieldHeaders , each Text.Contains( _, "Academic" ) or Text.Contains( _, "CurrentA" ) or Text.Contains( _, "PreviousA" ) or Text.Contains( _, "DateA" ) or Text.Contains( _, "AY" )), 
          AYCols = {
            "Academic Year",
            "Academic_Year",
            "Academic Quarter", 
          ...
          list_AllCols = List.Union({list_FiscalCols, list_AcademicCols}), 
          ListCols = 
            if var_FYAYTest then
              Table.RemoveColumns(cols_Reorder, list_AllCols)
            else if var_AYLogicTest then
              Table.RemoveColumns(cols_Reorder, list_AcademicCols)
            else if var_FYLogicTest then
              Table.RemoveColumns(cols_Reorder, list_FiscalCols)
            else
              Table.RemoveColumns(
                cols_Reorder, 
